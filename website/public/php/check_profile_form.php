...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (checkParameters($name, $surname, $email,$username, $errorMessage)) // se true, i dati sono nel formato corretto, provo ad autentificare l'utente
  {
    ...
    if ($man->register( $name, $surname, $email, $username)) // se registrato correttamente
    {
...
function checkParameters($name, $surname, $email,$username, &$errorMessage) // controlla il formato dei dati e ritorna true se sono corretti, altrimenti ritorna false e setta le stringhe di errore nella variabile errormessage
{
	if (empty($username)) {
